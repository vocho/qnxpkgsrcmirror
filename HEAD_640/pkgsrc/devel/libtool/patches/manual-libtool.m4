...
--- libtool.m4.orig	2007-06-23 22:19:11.000000000 -0400
+++ libtool.m4
@@ -248,7 +248,10 @@ AC_DEFUN([_LT_CC_BASENAME],
     *) break;;
   esac
 done
...
@@ -1871,9 +1894,7 @@ if test -f "$ltmain" && test -n "$tagnam
...
+  lt_cv_deplibs_check_method='match_pattern /lib[[^/]]+(\.so|S\.a)$'
 ...
 openbsd*)
@@ -2629,21 +2654,10 @@ AC_DEFUN([AC_LIBTOOL_CXX],
 # ---------------
...
-])# _LT_AC_PROG_CXXCPP
 ...
 # AC_LIBTOOL_F77
 # --------------
@@ -2781,7 +2795,7 @@ AC_DEFUN([AC_LIBTOOL_LANG_CXX_CONFIG], [
...
@@ -3294,6 +3308,16 @@ case $host_os in
...
+    ;;
   irix5* | irix6*)
     case $cc_basename in
       CC*)
@@ -3447,14 +3471,29 @@ case $host_os in
     ;;
   netbsd*)
     if echo __ELF__ | $CC -E - | grep __ELF__ >/dev/null; then
...
@@ -3767,6 +3806,11 @@ case $host_os in
...
+    ;;
   *)
     # FIXME: insert proper C++ library support
     _LT_AC_TAGVAR(ld_shlibs, $1)=no
@@ -3928,6 +3972,21 @@ if AC_TRY_EVAL(ac_compile); then
     esac
   done
...
@@ -4966,9 +5025,11 @@ AC_MSG_CHECKING([for $compiler option to
...
@@ -4994,6 +5055,10 @@ AC_MSG_CHECKING([for $compiler option to
 	;;
       esac
       ;;
+    interix3*)
...
+      ;;
     *)
       ...
       ;;
@@ -5072,6 +5137,8 @@ AC_MSG_CHECKING([for $compiler option to
...
+	;;
 	...
 	case $cc_basename in
 	  CC*)
@@ -5211,6 +5278,8 @@ AC_MSG_CHECKING([for $compiler option to
 	;;
       vxworks*)
 	;;
+      nto-qnx*)
+	;;
 	...
 	;;
@@ -5251,9 +5320,11 @@ AC_MSG_CHECKING([for $compiler option to
       ;;
...
+      _LT_AC_TAGVAR(lt_prog_compiler_static, $1)=''
     ...
     interix[[3-9]]*)
@@ -5287,6 +5358,11 @@ AC_MSG_CHECKING([for $compiler option to
       esac
       ;;
...
+
     *)
       _LT_AC_TAGVAR(lt_prog_compiler_pic, $1)='-fPIC'
       ;;
@@ -5441,6 +5517,10 @@ AC_MSG_CHECKING([for $compiler option to
