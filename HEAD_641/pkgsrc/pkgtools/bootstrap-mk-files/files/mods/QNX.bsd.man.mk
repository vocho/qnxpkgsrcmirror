...
MANTARGET?=	cat
NROFF?=		${PREFIX}/bin/nroff -Tascii
GROFF?=		${PREFIX}/bin/groff -Tascii
TBL?=		${PREFIX}/bin/tbl
...
.if !defined(USETBL)
.  if !exists(${NROFF:[1]})
	@echo "*** ${NROFF:[1]} not installed.  Can't make ${.TARGET} from ${.IMPSRC}"
	@echo "*** If not bootstrapping consider USE_TOOLS += nroff"
.  else
...
.  endif
.else
.  if (!exists(${NROFF:[1]}) || !exists(${TBL:[1]}))
	@echo "*** ${NROFF:[1]} not installed.  Can't make ${.TARGET} from ${.IMPSRC}"
	@echo "*** If not bootstrapping consider USE_TOOLS += nroff"
.  else
...
.if !defined(USETBL)
.  if !exists(${GROFF:[1]})
	@echo "*** ${GROFF:[1]} not installed.  Can't make ${.TARGET} from ${.IMPSRC}"
	@echo "*** If not bootstrapping consider USE_TOOLS += nroff"
.  else
...
.  endif
.else
.  if (!exists(${GROFF:[1]}) || !exists(${TBL:[1]}))
	@echo "*** ${GROFF:[1]} not installed.  Can't make ${.TARGET} from ${.IMPSRC}"
	@echo "*** If not bootstrapping consider USE_TOOLS += nroff"
.  else
